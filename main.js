...
function showSeatDetails(pos) {
    ...
    elPopup.querySelector('.btn-highlight').innerHTML = `<button class="btn-highlight"
    onclick="onHighlightSeatsAround(${pos.i},${pos.j})">Highlight</button>`
...
function onHighlightSeatsAround(rowIdx, colIdx) {
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        ...
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            ...
            if (currCell.isSeat && !currCell.isBooked) {
                var elCurrSeat = document.querySelector(`[data-i="${i}"][data-j="${j}"]`)
                elCurrSeat.classList.add('highlight')
                if (gHighlightTimeout) clearTimeout(gHighlightTimeout)
                gHighlightTimeout = setTimeout(removeHighlight, 2500)
            }
        }
    }
}
...
function removeHighlight() {
    var elHighlightedSeats = document.querySelectorAll('.highlight')
    for (var i = 0; i < elHighlightedSeats.length; i++) {
        var currSeat = elHighlightedSeats[i]
        currSeat.classList.remove('highlight')
    }
}
